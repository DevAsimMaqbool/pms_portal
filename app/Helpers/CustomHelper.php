...
function RatingonImpact($facultyId, $activeRoleId, $currentYear = null)
{
    // 1️⃣ Get all knowledge products created by the faculty
    $knowledgeProducts = \App\Models\ResearchConferenceImpact::where('created_by', $facultyId)
        ->where('status', 3) // optional: only approved/active products
        ->where('year_id', $currentYear)
        ->get();
    ...
    // 2️⃣ Get the target from faculty_targets table
    $targetRecord = FacultyTarget::where('user_id', $facultyId)
        ->where('indicator_id', 140) // indicator_id for NumberOfKnowledgeProduct
        ->where('year_id', $currentYear)
        ->first();
    ...
    $weights = [
        'course_load' => getRoleWeightage($activeRoleId, 'indicator', 140)['weightage'],
    ];
    $weightedScore = ($score * $weights['course_load']) / 100;
    saveIndicatorPercentage($facultyId, $activeRoleId, 2, 9, 140, $weightedScore, $score, $currentYear);
...
function StudentAttendanceOfHOD($employeeId, $activeRoleId)
{
    ...
    /*
    |--------------------------------------------------------------------------
    | Check whether this HOD manages multiple departments
    |--------------------------------------------------------------------------
    */

    $managedDepartmentIds = User::where('manager_id', $user->id)
    ...
    */
    ...
    if ($managedDepartmentIds->count() > 1) {

        $facultyMembers = User::whereIn('department_id', $managedDepartmentIds)
        ...
        /*
        |--------------------------------------------------------------------------
        | Existing Single Department HOD Logic
        |--------------------------------------------------------------------------
        */
    ...
    foreach ($facultyMembers as $faculty) {
        ...
        foreach ($classes as $class) {
            ...
            $data[] = (object) [
                ...
                // Will be available for multi-department HOD
                // and null for existing single-department flow
...
function StudentSatisfactionRateForHOD($activeRoleId, $indicatorId,$currentYear=null)
{
    $departmentId = auth()->user()->department_id;
...
if (!function_exists('departmentTargetIndicatorsAnalysisOfHOD')) {
    ...
    function departmentTargetIndicatorsAnalysisOfHOD($employeeId, $activeRoleId, $KpaId, $categoryId, $indicatorId, $currentYear = null)
    {
        ...
        $indicators = [
            ...
            140 => [
                'model' => \App\Models\ResearchConferenceImpact::class,
                'status' => 3
            ],
...
if (!function_exists('scholarsSatisfactionAverageOfHOD')) {
    function scholarsSatisfactionAverageOfHODbk($activeRoleId)
    {
        $departmentId = auth()->user()->department_id;
        $currentYear = Carbon::now()->year;
        $previousYear = Carbon::now()->year - 1;
    ...
    function scholarsSatisfactionAverageOfHOD($activeRoleId)
{
    ...
    $activeTerms = Term::where('status', '1')
        ->orderBy('id', 'desc')
        ->get(['id', 'term', 'start_year']);

    $activeTermIds = $activeTerms->pluck('id');

    // Get records
    $recordsRaws = ScholarsSatisfactionInThesisStage::where('department_id', $departmentId)
        ->where('indicator_id', 132)
        ->whereIn('term_id', $activeTermIds)
        ->with([
            'faculty',
    ...
    /*
    |--------------------------------------------------------------------------
    | Group records by term_id
    |--------------------------------------------------------------------------
    */
    $recordsRaw = $recordsRaws->groupBy('term_id');
    ...
    /*
    |--------------------------------------------------------------------------
    | Loop through term_id groups
    |--------------------------------------------------------------------------
    */
    foreach ($recordsRaw as $termId => $termRecords) {

        $firstRecord = $termRecords->first();

        if (!$firstRecord || !$firstRecord->terms) {
            continue;
        }

        $termName = $firstRecord->terms->term ?? '';
        $termNameLower = strtolower($termName);

        /*
        |--------------------------------------------------------------------------
        | Determine Spring / Fall
        |--------------------------------------------------------------------------
        */
        if (str_contains($termNameLower, 'spring')) {
            $season = 'Spring';
        } elseif (str_contains($termNameLower, 'fall')) {
            $season = 'Fall';
        } else {
            continue;
        }
        ...
        |--------------------------------------------------------------------------
        */
        $records[$season]['term_id'] = $termId;
        $records[$season]['term_name'] = $termName;
        $records[$season]['term_year'] = $firstRecord->terms->start_year;

        /*
        |--------------------------------------------------------------------------
        | Group this term's records by program_id
        |--------------------------------------------------------------------------
        */
        $programGroups = $termRecords->groupBy('program_id');

        foreach ($programGroups as $programId => $programRecords) {

            $firstProgramRecord = $programRecords->first();
            ...
            |--------------------------------------------------------------------------
            */
            $programSum = $programRecords->sum(function ($record) {
                return (float) ($record->satisfaction_score ?? 0);
            });

            $programCount = $programRecords->count();

            $programAvg = $programCount > 0
                ? round($programSum / $programCount, 2)
                : 0;
            ...
            |--------------------------------------------------------------------------
            */
            $records[$season]['sum'] += $programSum;
            $records[$season]['count'] += $programCount;
            ...
            |--------------------------------------------------------------------------
            */
            $records[$season]['details'][] = [
                'program_id' => $programId,

                'faculty_id' => $firstProgramRecord->faculty_id,
                'faculty_name' => $firstProgramRecord->faculty->name ?? 'N/A',

                'department_id' => $firstProgramRecord->department_id,
                'department_name' => $firstProgramRecord->department->name ?? 'N/A',

                'program_name' => $firstProgramRecord->program->program_name ?? 'N/A',

                'program_level' => $firstProgramRecord->career ?? 'N/A',

                'sum' => $programSum,
                'count' => $programCount,
                'avg' => $programAvg,

                'satisfaction_score' => $programAvg,

                'term_id' => $termId,
                'term_name' => $termName,
                'term_year' => $firstRecord->terms->start_year,
            ];
        }
    }
    ...
    /*
    |--------------------------------------------------------------------------
    | Calculate Spring / Fall averages
    |--------------------------------------------------------------------------
    */
    foreach (['Spring', 'Fall'] as $season) {

        if ($records[$season]['count'] > 0) {

            $records[$season]['avg'] = round(
                $records[$season]['sum'] /
    ...
    */
    //$totalTargetSum = $recordsRaw->sum('satisfaction_score');
    $totalTargetSum = $recordsRaw->flatten(1)->sum('satisfaction_score');

    $totalTargetCount = $recordsRaw->count();
    //dd($totalTargetSum);

    $totalTargetAvg = $totalTargetCount > 0
        ? round($totalTargetSum / $totalTargetCount, 2)
        : 0;
...
if (!function_exists('researchProductivityPGStudentsOfHOD')) {
    function researchProductivityPGStudentsOfHOD($activeRoleId, $indicatorId, $keyPerformanceAreaId = 2, $indicatorCategoryId = 6)
    {
        ...
        // =======================
        // ✅ NEW: Department Rows for Modal
        // =======================

        $groupedData = [];

        $allRecords = ResearchProductivityOfPgStudentTarget::whereIn('created_by', $facultyIds)
            ->where('indicator_id', $indicatorId)
            ->where('form_status', 'RESEARCHER')
            ->where('status', 3)
            ->get();
        ...
        $grouped = $allRecords->groupBy(function ($item) {
            return $item->target_category . '|' . $item->journal_clasification;
        });

        foreach ($grouped as $key => $records) {

            [$category, $journal] = explode('|', $key);

            $target = FacultyTarget::whereIn('user_id', $facultyIds)
                ->where('indicator_id', $indicatorId)
                ->where('form_status', 'HOD')
                ->sum('target');

            $achieved = $records->count();

            $percentage = $target > 0 ? ($achieved / $target) * 100 : 0;

            $groupedData[] = [
                'target_category' => $category,
                'journal_clasification' => $journal,
                'value' => $target,
                'count' => $achieved,
                'percentage' => $percentage
            ];
...
if (!function_exists('admissionTargetDepartmentAverage')) {
    function admissionTargetDepartmentAveragebbk($employeeId, $activeRoleId, $indicatorId)
    {
        ...
        $recordsRaw = AdmissionTargetAchieved::with(['faculty', 'department', 'program'])
            ...
            ->groupBy('admissions_campaign')
            ->get();
        $records = [
            'Spring' => ['total_target' => 0, 'total_achieved' => 0, 'percentage' => 0],
            'Fall' => ['total_target' => 0, 'total_achieved' => 0, 'percentage' => 0],
        ];
        foreach ($recordsRaw as $record) {
            $totalTarget1 = $record->total_target ?? 0;
            $totalAchieved1 = $record->total_achieved ?? 0;
            $percentage = $totalTarget1 > 0 ? ($totalAchieved1 / $totalTarget1) * 100 : 0;

            if (str_contains($record->admissions_campaign, 'Spring')) {
                $records['Spring']['total_target'] = $totalTarget1;
                $records['Spring']['total_achieved'] = $totalAchieved1;
                $records['Spring']['percentage'] = round($percentage, 1);
            } else if (str_contains($record->admissions_campaign, 'Fall')) {
                $records['Fall']['total_target'] = $totalTarget1;
                $records['Fall']['total_achieved'] = $totalAchieved1;
...
if (!function_exists('recoveryTargetDepartmentAveraget')) {
    function recoveryTargetDepartmentAveraget($employeeId, $activeRoleId, $indicatorId)
    {
        ...
        $recordsRaw = Recovery::with(['faculty', 'department', 'program'])
            ->select(
                'program_id',
                \DB::raw('SUM(recovery_target) as total_target'),
                \DB::raw('SUM(achieved_target) as total_achieved')
            )
            ->where('department_id', $departmentId)
            ->groupBy('program_id')
        ...
        foreach ($recordsRaw as $record) {
            $totalTarget1 = $record->total_target ?? 0;
            $totalAchieved1 = $record->total_achieved ?? 0;
            $programPercentage = $totalTarget1 > 0 ? ($totalAchieved1 / $totalTarget1) * 100 : 0;

            $records[] = [
                'program_name' => $record->program->program_name ?? 'N/A',
                'total_target' => $totalTarget1,
                'total_achieved' => $totalAchieved1,
...
if (!function_exists('programProfitabilityDepartmentAverage')) {
    function programProfitabilityDepartmentAverage($employeeId, $activeRoleId, $indicatorId)
    {
        ...
        $recordsRaw = ProgramProfitability::with(['faculty', 'department', 'program'])
            ->select(
                'program_id',
                \DB::raw('SUM(profitability) as total_profitability'),
                \DB::raw('AVG(profitability) as avg_profitability'),
            )
            ->where('department_id', $departmentId)
            ->groupBy('program_id')
        ...
        $totalProfitability = $recordsRaw->sum('total_profitability');
        $avgtotalProfitability = $recordsRaw->AVG('total_profitability');
        $indicatorWeight = getRoleWeightage($activeRoleId, 'indicator', $indicatorId);
        $weight = $indicatorWeight['weightage'] ?? 0;

        // Weighted score
        $weightedScore = ($avgtotalProfitability * $weight) / 100;
...
if (!function_exists('lineManagerReviewRatingOnTasks169')) {
    function lineManagerReviewRatingOnTasks169($employeeId, $activeRoleId)
    {
        ...
        return ['average_score' => $averageScore, 'total_score' => $totalScore, 'ratings' => $managerRatings];
    }
}
if (!function_exists('facultyPursuingSkills')) {
    function facultyPursuingSkills($employeeId, $activeRoleId)
    {
        $departmentId = auth()->user()->department_id;
        ...
        $employeeIds = User::where('department_id', $departmentId)
            ...
            ->filter() // remove nulls
            ->toArray();
        $totalUserCount = count($employeeIds);

        if (empty($employeeIds)) {
            return ['average_score' => 0, 'total_user' => 0, 'total_fill_user' => 0, 'weighted_score' => 0];
        }

        $FacultyPursuingSkillcount = FacultyPursuingSkill::whereIn('created_by', $employeeIds)
            ->where('status', 2)
            ->distinct('created_by')
            ->count('created_by');

        // 3️⃣ Department-level average
        $averageScore = $FacultyPursuingSkillcount > 0 ? round($totalUserCount / $FacultyPursuingSkillcount, 2) : 0;

        $weight = getRoleWeightage($activeRoleId, 'indicator', 170)['weightage'] ?? 0;
        ...
        return ['average_score' => $averageScore, 'total_user' => $totalUserCount, 'total_fill_user' => $FacultyPursuingSkillcount, 'weighted_score' => $weightedScore];
    }
}
