        ...
        <div class="card">
             <div class="card-header d-flex align-items-center justify-content-between">
                <div class="card-title mb-0">
                    <h5 class="mb-1">Research Tasks Assigned by Dean / HOD</h5>
                </div>
                <div class="">
                    <a href="{{ url('kpa/2/category/43/indicator/175') }}" class="btn btn-success">Add</a>
                </div>
            </div>
...
@push('script')
    @if(in_array(getRoleName(activeRole()), ['HOD','Dean']))
        <script>
                ...
                $.ajax({
                    url: "{{ route('research-tasks-assigned.index') }}",
                    method: "GET",
                    data: {
...
$(document).ready(function(){
    ...
    $('#researchForm1').on('submit', function (e) {
        ...
        $.ajax({
            url: "{{ route('research-tasks-assigned.update', '') }}/" + recordId,
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $.ajax({
        url: `/research-tasks-assigned/${id}`,
        type: 'DELETE',
