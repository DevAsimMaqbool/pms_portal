...
@if(in_array(getRoleName(activeRole()), ['Dean']))
    ...
    $data = ResearchInnovationAndCommercializationYear(Auth::user()->employee_id, $activeRoleId, 1, 1, 106);
...
@if(in_array(getRoleName(activeRole()), ['Program Leader UG', 'Program Leader PG']))
    ...
    $avgAbove = $programBreakdown->sum('above') ?? 0;
    $avgAtPar = $programBreakdown->sum('at_par') ?? 0;
    $avgLow = $programBreakdown->sum('low') ?? 0;
        ...
        $rating = $meta->rating ?? '-';
                                    ...
                                    <tfoot>
                                        <tr class="table-primary">
                                            ...
                                            <!-- Avg Above -->
                                            <th>{{ number_format($avgAbove, 1) }}</th>

                                            <!-- Avg At Par -->
                                            <th>{{ number_format($avgAtPar, 1) }}</th>

                                            <!-- Avg Low -->
                                            <th>{{ number_format($avgLow, 1) }}</th>
