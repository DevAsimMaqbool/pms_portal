        ...
        <table class="table">
            ...
            <tbody>
                ...
                @foreach($record as $row)
                    @php
                        $totalObtained += $row->obtained_score;
                        $totalMax += $row->max_score;
                        $rowPercentage = $row->max_score > 0
                            ? number_format(($row->obtained_score / $row->max_score) * 100, 2)
                            : 0;
                    @endphp
                ...
                <tr style="font-weight: bold; background-color: #e0f7fa;">
                    ...
                    <td>
                        @if($totalMax > 0)
                            {{ number_format(($totalObtained / $totalMax) * 100, 2) }}%
                        @else
                            0%
                        @endif
                    </td>
