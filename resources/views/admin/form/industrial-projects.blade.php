        ...
        <div class="card">
            <div class="card-datatable table-responsive card-body">
                ...
                <div class="tab-content">
                    {{-- ================= FORM 1 ================= --}}
                    @if(in_array(getRoleName(activeRole()), ['HOD','Teacher','Associate Professor','Assistant Professor','Professor']))
 ...
 @if(in_array(getRoleName(activeRole()), ['HOD']))
        <script>
                        ...
                        const rowData = forms.map((form, i) => {
                            ...
                            if (form.status == 1){
                                if (form.reject_status == 1){
                                    statusText = '<span class="badge bg-label-danger">Reject by HOD</span>';
                                }else if (form.reject_status == 2){
                                    statusText = '<span class="badge bg-label-danger">Reject by ORIC</span>';
                                }else{
            ...
            function updateSingleStatus(id, status) {
                $.ajax({
                    url: `/industrial-projects/${id}`,           // single row endpoint
                    type: 'POST',                            // POST with _method PUT
                    data: {
                        _method: 'PUT',
                        _token: $('meta[name="csrf-token"]').attr('content'),
            ...
            $(document).ready(function () {
                ...
                $(document).on('click', '.view-form-btn', function () {
                    ...
                    if (window.activeUserRole === 'HOD') {
                        ...
                        if (form.reject_status == 2 || form.reject_status == 1) {    
                             // Append to cell
                    ...
                    if (form.contracting_industry) {
                        $('#modalExtraFields').append(`<tr class="optional-field"><th>Contracting Industry</th><td>${form.contracting_industry}</td></tr>`);
                    }
                    if (form.project_duration) {
                        $('#modalExtraFields').append(`<tr class="optional-field"><th>project Duration</th><td>${form.project_duration}</td></tr>`);
                    }
                    if (form.estimated_project_cost) {
                        $('#modalExtraFields').append(`<tr class="optional-field"><th>Estimated Project Cost</th><td>${form.estimated_project_cost}</td></tr>`);
                    }
                    if (form.estimated_complection) {
                        $('#modalExtraFields').append(`<tr class="optional-field"><th>Estimated Complection</th><td>${form.estimated_complection}</td></tr>`);
                    }
            ...
            $(document).ready(function () {
                ...
                $(document).on('change', '.status-radio', function () {
                    ...
                    if (value === 'reject') {
                        ...
                        // Approve directly
                        updateSingleStatus(id, 3); // 2 for approve
                    }
                });
