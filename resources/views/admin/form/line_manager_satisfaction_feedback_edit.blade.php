                                ...
                                <form id="researchForm" class="row" method="POST"
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select name="employee_id" class="select2 form-select" required disabled>
                                                ...
                                                @foreach($facultyMembers as $member)
                                                    <option value="{{ $member->id }}" {{ $rating->employee_id == $member->id ? 'selected' : '' }}>
                                                        {{ $member->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="row" style="padding:20px; display:flex; flex-wrap:wrap; gap:15px;">
                                        ...
                                        <div class="col-md-6 mt-1"
                                            ...
                                            <label style="font-weight:bold; margin-bottom:8px;">Admits mistakes and takes responsibility for
                                                correcting them</label>
                                            <div style="margin-top:8px;">
                                                @foreach([20 => 'Strongly Disagree', 40 => 'Disagree', 60 => 'Neutral', 80 => 'Agree', 100 => 'Strongly Agree'] as $val => $label)
                                                    <label style="margin-right:15px;">
                                                        <input type="radio" name="responsibility_accountability_1" value="{{ $val }}" {{ $rating->responsibility_accountability_1 == $val ? 'checked' : '' }}> {{ $label }}
                                                    </label>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6 mt-1"
                                            style="flex:1 1 calc(50% - 15px); padding:15px; border:1px solid #ddd; border-radius:6px;">
                                            <label style="font-weight:bold; margin-bottom:8px;">Keeps commitments and meets deadlines
                                                consistently</label>
                                            <div style="margin-top:8px;">
                                                @foreach([20 => 'Strongly Disagree', 40 => 'Disagree', 60 => 'Neutral', 80 => 'Agree', 100 => 'Strongly Agree'] as $val => $label)
                                                    <label style="margin-right:15px;">
                                                        <input type="radio" name="responsibility_accountability_2" value="{{ $val }}" {{ $rating->responsibility_accountability_2 == $val ? 'checked' : '' }}> {{ $label }}
                                                    </label>
                                                @endforeach
                                            </div>
                                        </div>
        ...
        'inspirational_leadership' => '5- Inspirational Leadership'
                                            ...
                                            @for($i = 1; $i <= 2; $i++)
                                                <div class="col-md-6 mt-1"
                                                    ...
                                                    <div style="margin-top:8px;">
                                                        @foreach([20 => 'Strongly Disagree', 40 => 'Disagree', 60 => 'Neutral', 80 => 'Agree', 100 => 'Strongly Agree'] as $val => $label)
                                                            <label style="margin-right:15px;">
                                                                <input type="radio" name="{{ $key }}_{{ $i }}" value="{{ $val }}" {{ $rating->{$key . '_' . $i} == $val ? 'checked' : '' }}> {{ $label }}
                                                            </label>
                                                        @endforeach
                                                    </div>
                                                </div>
                                        ...
                                        <div style="width:100%; margin-top:30px; margin-bottom:20px;">
                                            ...
                                            <textarea name="remarks"
                                                style="width:100%; height:120px; padding:10px; border:1px solid #ccc; border-radius:6px;" required
                                        placeholder="Please provide details of the assigned task(s) and the employee’s designated role and responsibilities during the event.">{{ $rating->remarks }}</textarea>
                                        </div>
