            ...
            <div class="card-datatable table-responsive card-body">
                @if(auth()->user()->hasRole(['Dean']))
                    ...
                    <ul class="nav nav-tabs mb-3" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" data-bs-toggle="tab" href="#form1" role="tab">Products Delivered to Industry</a>
                        </li>
                        <li class="nav-item">
                    ...
                    @if(auth()->user()->hasRole(['HOD', 'Teacher', 'Assistant Professor','Professor']))
                        ...
                        <div class="d-flex justify-content-between">
                               <div>
                                ...
                                <a href="{{ route('indicators_crud.index', ['slug' => 'products-delivered-to-industry', 'id' => $indicatorId]) }}" class="btn rounded-pill btn-outline-primary waves-effect"> View</a>
                            </div> 
                            <h5 class="mb-1"></h5>
    ...
    @if(auth()->user()->hasRole(['HOD']))
        <script>
            function fetchIndicatorForms3() {
                $.ajax({
                    url: "{{ route('products-delivered-to-industry.index') }}",
                    method: "GET",
                    data: {
            ...
            function updateSingleStatus(id, status) {
                $.ajax({
                    url: `/products-delivered-to-industry/${id}`,           // single row endpoint
